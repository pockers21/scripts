...
def execute1(batch_size, max_seq_length, load_bits):
    ...
    #attention_mask = torch.cat((attention_mask, torch.ones((attention_mask.size(0), 1), dtype=attention_mask.dtype)),
    #                           dim=-1)
    with torch.no_grad():
    ...
    total_time_remaining_tokens = end_time_remaining_tokens - start_time_remaining_tokens - ttft

    total_tokens = output.sequences.numel()
    tpot = total_time_remaining_tokens / (total_tokens-6) if total_time_remaining_tokens > 0 else float('inf')
...
def validate_param():
    ...
    bs_seq_len_set = {(1, 128), (4, 128), (1, 2048), (2, 128)}
